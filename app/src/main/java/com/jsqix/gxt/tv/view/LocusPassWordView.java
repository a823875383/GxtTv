...
import com.jsqix.gxt.tv.obj.Point;
import com.jsqix.gxt.tv.utils.MathUtil;
...
public class LocusPassWordView extends View {
	...
	private long CLEAR_TIME = 1000;// 清除痕迹的时间
	private int pwdMaxLen = 9;// 密码最大长度
			...
			if (this.sPoints.size() > 0)
				if (this.sPoints.size() == 1) {
					this.reset();
						...
						|| sPoints.size() > pwdMaxLen) {
					// mCompleteListener.onPasswordTooMin(sPoints.size());
					error();
					clearPassword();
					Toast.makeText(this.getContext(),
							"password too short or too long, cannot be saved!",
	...
	public void setPasswordMinLength(int passwordMinLength) {
		this.pwdMinLen = passwordMinLength;
	}
